...
@dataclass
class SamleidProofingFunctions[BaseSessionInfoVar: BaseSessionInfo](ProofingFunctions[BaseSessionInfoVar]):
...
@dataclass
class BankIDProofingFunctions(SamleidProofingFunctions[NinSessionInfo]):
    def get_identity(self, user: User) -> IdentityElement | None:
    ...
    def identity_proofing_element(self, user: User) -> ProofingElementResult:
        ...
        data = BankIDProofing(
            ...
            transaction_id=self.session_info.attributes.transaction_id or "",
        )
    ...
    def mark_credential_as_verified(self, credential: Credential, loa: str | None) -> VerifyCredentialResult:
        if loa != "uncertified-loa3":
            return VerifyCredentialResult(error=SamleidMsg.authn_context_mismatch)
...
@dataclass
class FrejaProofingFunctions(SamleidProofingFunctions[NinSessionInfo]):
    def get_identity(self, user: User) -> IdentityElement | None:
        return user.identities.nin
...
@dataclass()
class EidasProofingFunctions(SamleidProofingFunctions[ForeignEidSessionInfo]):
    def get_identity(self, user: User) -> IdentityElement | None:
    ...
    def verify_identity(self, user: User) -> VerifyUserResult:
        ...
        if acc_loa is None or acc_loa.result is None:
            return VerifyUserResult(error=SamleidMsg.authn_context_mismatch)
