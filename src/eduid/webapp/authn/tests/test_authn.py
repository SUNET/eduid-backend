        ...
        config.update(
            {
                ...
                "frontend_action_authn_parameters": {
                    FrontendAction.OLD_LOGIN.value: {
                        "same_user": False,
                        "finish_url": "https://example.com/profile/",
                    },
...
# test for old behavior, should be removed as soon frontend have switched to the new way


class OldAuthnAPITestBase(EduidAPITestCase):
    """Test cases for the real eduid-authn app"""
...
class OldAuthnAPITestCase(OldAuthnAPITestBase):
    """
    Tests to check the different modes of authentication.
    """
...
class OldUnAuthnAPITestCase(EduidAPITestCase):
    """Tests for a fictitious app based on AuthnBaseApp"""
...
class OldNoAuthnAPITestCase(EduidAPITestCase):
    """Tests for a fictitious app based on AuthnBaseApp"""
...
class OldLogoutRequestTests(OldAuthnAPITestBase):
    def test_metadataview(self):
        with self.app.test_client() as c:
            response = c.get("/saml2-metadata")
            self.assertEqual(response.status, "200 OK")
