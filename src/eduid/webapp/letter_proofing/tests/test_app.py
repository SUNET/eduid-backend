...
class LetterProofingTests(EduidAPITestCase[LetterProofingApp]):
    ...
    def get_code_backdoor(
        ...
        with self.session_cookie(self.browser, self.test_user_eppn) as client:
            ...
            if add_cookie:
                ...
                client.set_cookie(
                    server_name="localhost", domain="localhost", key=self.app.conf.magic_cookie_name, value=cookie_value
                )
    ...
    def test_verify_letter_code_bad_csrf(self):
        with self.app.test_request_context():
            ...
            self._check_error_response(
                response,
                type_="POST_LETTER_PROOFING_VERIFY_CODE_FAIL",
    ...
    def test_verify_letter_code_fail(self):
        with self.app.test_request_context():
            ...
            self._check_error_response(
                response, type_="POST_LETTER_PROOFING_VERIFY_CODE_FAIL", msg=LetterMsg.wrong_code
            )
    ...
    def test_proofing_with_a_verified_nin(self):
        ...
        with self.app.test_request_context():
            user = self.app.central_userdb.get_user_by_eppn(self.test_user_eppn)
            verified_nin = NinIdentity(
                number=self.test_user_nin, created_by="test", is_verified=True, verified_by="test"
            )
