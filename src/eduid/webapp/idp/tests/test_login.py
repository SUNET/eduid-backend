...
class IdPTestLoginAPI(IdPAPITests):
    ...
    def test_login_pwauth_right_password_managed_account(self) -> None:
        eppn = "ma-12345678"
        managed_account = ManagedAccount(eppn=eppn)
        password = Password(
            credential_id=str(ObjectId("145645678901234567890456")),
            salt="$NDNv1H1$9c810d852430b62a9a7c6159d5d64c41c3831846f81b6799b54e1e8922f11545$32$32$",
            created_by="test",
            created_ts=datetime.fromisoformat("2022-01-24T08:19:25"),
            is_generated=True,
        )
        managed_account.credentials.add(password)
        self.app.managed_account_db.save(managed_account)
        test_user = TestUser(eppn=eppn, password="secret")

        # Patch the VCCSClient, so we do not need a vccs server
        with patch.object(VCCSClient, "authenticate") as mock_vccs:
            mock_vccs.return_value = True
            result = self._try_login(test_user=test_user)
        ...
        assert "eduPersonPrincipalName" in attributes
        assert attributes["eduPersonPrincipalName"] == [f"{eppn}@{self.app.conf.default_eppn_scope}"]
