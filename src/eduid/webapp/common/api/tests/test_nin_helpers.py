...
class NinHelpersTest(ProofingTests[HelpersTestApp]):
    ...
    def test_add_nin_to_user(self, mocker: MockerFixture) -> None:
        ...
        nin_element = NinProofingElement.from_dict(
            {"number": self.test_user_nin, "created_by": "NinHelpersTest", "verified": False}
        )
        proofing_state = NinProofingState.from_dict({"eduPersonPrincipalName": user.eppn, "nin": nin_element.to_dict()})
        with self.app.app_context():
    ...
    def test_verify_nin_for_user_existing_locked_not_verified(self, mocker: MockerFixture) -> None:
        ...
        user = ProofingUser.from_user(user, self.app.private_userdb)
        nin_element = NinProofingElement.from_dict(
            {"number": self.locked_test_user_nin, "created_by": "NinHelpersTest", "verified": False}
        )
        proofing_state = NinProofingState.from_dict({"eduPersonPrincipalName": user.eppn, "nin": nin_element.to_dict()})
        assert nin_element.created_by is not None
        proofing_log_entry = self._get_nin_eid_proofing_log_entry(
    ...
    def test_verify_wrong_nin_for_user_existing_not_verified(self, mocker: MockerFixture) -> None:
        ...
        user = ProofingUser.from_user(user, self.app.private_userdb)
        nin_element = NinProofingElement.from_dict(
            {"number": self.wrong_test_user_nin, "created_by": "NinHelpersTest", "verified": False}
        )
        proofing_state = NinProofingState.from_dict({"eduPersonPrincipalName": user.eppn, "nin": nin_element.to_dict()})
        assert proofing_state.nin.created_by is not None
        assert nin_element.created_by
    ...
    def test_set_user_names_from_official_address_middle_name(self) -> None:
        user = ProofingUser.from_dict(data=self.test_userdata)
        navet_response = self.navet_response()
        navet_response.name.middle_name = "Mellan"
        proofing_element = self._get_nin_navet_proofing_log_entry(
            user=user,
            created_by="test",
            nin="190102031234",
        ...
        with self.app.app_context():
            ...
            assert user.given_name == "Testaren Test"
            assert user.chosen_given_name == "Test"
            assert user.surname == "Mellan Testsson"
            assert user.legal_name == "Testaren Test Mellan Testsson"

    def test_set_user_names_from_official_address_middle_name_whitespace(self) -> None:
        user = ProofingUser.from_dict(data=self.test_userdata)
        navet_response = self.navet_response()
        navet_response.name.middle_name = "  "
        proofing_element = self._get_nin_navet_proofing_log_entry(
            user=user,
            created_by="test",
            nin="190102031234",
    ...
    def test_set_user_names_from_official_address_blank_surname(self) -> None:
        user = ProofingUser.from_dict(data=self.test_userdata)
        navet_response = self.navet_response()
        navet_response.name.surname = ""
        navet_response.name.middle_name = None
        proofing_element = self._get_nin_navet_proofing_log_entry(
            user=user,
            created_by="test",
            nin="190102031234",
    ...
    def test_get_official_surname() -> None:
        cases: list[tuple[Name, str | None]] = [
            (Name(surname="Testsson"), "Testsson"),
            (Name(middle_name="Mellan", surname="Testsson"), "Mellan Testsson"),
            (Name(middle_name="   ", surname="Testsson"), "Testsson"),
            (Name(middle_name=" Mellan ", surname=" Testsson "), "Mellan Testsson"),
            (Name(middle_name="Mellan", surname=""), "Mellan"),
            (Name(), None),
        ]
        for name, expected in cases:
            assert get_official_surname(name) == expected, f"Failed for {name}"
