...
def add_password(
    ...
    if not vccs.add_credentials(str(user.user_id), [new_factor]):
        logger.error(f"Failed adding password version {version} credential {new_factor.credential_id} for user {user}")
        return False  # something failed
    logger.info(f"Added password version {version} credential {new_factor.credential_id} for user {user}")
...
def upgrade_password_to_v2(
    ...
    if not add_password(
        ...
        logger.error(f"Failed upgrading password credential for user {user}")
        return False

    logger.info(f"Upgraded password credential for user {user} (upgraded from {old_credential.key})")
    return True
...
def change_password(
    ...
    if version == 2:
        revoke_passwords(user=user, reason=reason, application=application, vccs_url=vccs_url, vccs=vccs)
...
def reset_password(
    ...
    # change password with no old password is equal to a password reset
