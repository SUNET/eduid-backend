...
class TestGroupResource(ScimApiTestCase):
    ...
    def _perform_search(
        ...
        response = self.client.post(url="/Groups/.search", data=req, headers=self.headers)
...
class TestGroupResource_POST(TestGroupResource):
    def test_create_group(self):
        ...
        response = self.client.post(url="/Groups/", data=req, headers=self.headers)
    ...
    def test_schema_violation(self):
        ...
        response = self.client.post(url=f"/Groups/", data=req, headers=self.headers)
...
class TestGroupResource_PUT(TestGroupResource):
    def test_update_group(self):
        ...
        response = self.client.put(url=f"/Groups/{db_group.scim_id}", data=req, headers=self.headers)
    ...
    def test_update_group_not_found(self):
        ...
        response = self.client.put(url=f'/Groups/{req["id"]}', data=req, headers=self.headers)
        self._assertScimError(response.json(), status=404, detail="Group not found")
    ...
    def test_schema_violation(self):
        ...
        response = self.client.put(url=f"/Groups/{uuid4()}", data=req, headers=self.headers)
        self._assertScimError(
            status=422,
            json=response.json(),
            detail=[{"loc": ["body", "displayName"], "msg": "field required", "type": "value_error.missing"}],
...
class TestGroupExtensionData(TestGroupResource):
    def test_nutid_extension(self):
        ...
        post_resp = self.client.post(url="/Groups/", data=req, headers=self.headers)
        ...
        put_resp = self.client.put(url=f"/Groups/{scim_id}", data=req, headers=self.headers)
