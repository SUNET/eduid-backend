...
class MsgRelayTests:
    ...
    def test_get_all_navet_data(self, mocker: MockerFixture) -> None:
        ...
        mock_get_all_navet_data.return_value.get.return_value = self.message_sender.get_devel_all_navet_data()
        res = self.msg_relay.get_all_navet_data(nin="190102031234")
        assert res == NavetData(**self.message_sender.get_devel_all_navet_data())

    def test_get_all_navet_data_deceased(self, mocker: MockerFixture) -> None:
        mock_get_all_navet_data = mocker.patch("eduid.workers.msg.tasks.get_all_navet_data.apply_async")
        mock_get_all_navet_data.return_value.get.return_value = self.message_sender.get_devel_all_navet_data(
            identity_number="189001019802"
        )
    ...
    def test_get_all_navet_data_none_response(self, mocker: MockerFixture) -> None:
        mock_get_all_navet_data = mocker.patch("eduid.workers.msg.tasks.get_all_navet_data.apply_async")
        mock_get_all_navet_data.return_value.get.return_value = None
        with pytest.raises(NoNavetData):
            self.msg_relay.get_all_navet_data(nin="190102031234")

    def test_get_postal_address(self, mocker: MockerFixture) -> None:
        mock_get_postal_address = mocker.patch("eduid.workers.msg.tasks.get_postal_address.apply_async")
        mock_get_postal_address.return_value.get.return_value = self.message_sender.get_devel_postal_address()
    ...
    def test_get_postal_address_none_response(self, mocker: MockerFixture) -> None:
        mock_get_postal_address = mocker.patch("eduid.workers.msg.tasks.get_postal_address.apply_async")
        mock_get_postal_address.return_value.get.return_value = None
        with pytest.raises(NoAddressFound):
            self.msg_relay.get_postal_address(nin="190102031234")

    def test_get_relations_to(self, mocker: MockerFixture) -> None:
        mock_get_relations = mocker.patch("eduid.workers.msg.tasks.get_relations_to.apply_async")
        relations_to = self._fix_relations_to(
            relative_nin="194004048989", relations=self.message_sender.get_devel_relations()
        )
        mock_get_relations.return_value.get.return_value = relations_to
    ...
    def test_get_relations_to_empty_response(self, mocker: MockerFixture) -> None:
        mock_get_relations = mocker.patch("eduid.workers.msg.tasks.get_relations_to.apply_async")
        mock_get_relations.return_value.get.return_value = []
        res = self.msg_relay.get_relations_to(nin="190102031234", relative_nin="194004048989")
        assert res == []
