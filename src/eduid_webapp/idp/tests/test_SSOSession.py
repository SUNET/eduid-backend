...
class test_SSOSession(IdPTests):
    ...
    def test_from_dict(self):
        session = SSOSession.from_dict(self.data['data'], self.app.userdb)
    ...
    def test_with_datetime_authn_timestamp(self):
        int_session = SSOSession.from_dict(self.data['data'], self.app.userdb)
        data = dict(self.data)
        # Change authn_timestamp to datetime format.
        data['authn_timestamp'] = datetime.fromisoformat('2020-09-13T12:26:40+00:00')
        datetime_session = SSOSession.from_dict(data['data'], self.app.userdb)
