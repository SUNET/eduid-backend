...
VOLUME ["/opt/eduid/run", "/opt/eduid/src", "/var/log"]
...
RUN (cd /opt/eduid/eduid-queue; git describe --all; git log -n 1) > /revision.txt
RUN rm -rf /opt/eduid/eduid-queue/.git

WORKDIR "/opt/eduid/eduid-queue"

ENTRYPOINT ["/opt/eduid/eduid-queue/docker/start.sh"]
